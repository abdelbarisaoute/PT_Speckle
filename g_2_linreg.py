...
plt.plot(np.arange(0, 95) / 120, g_full, "-o", label=r"$g_2(\tau)-1$")
plt.axhline(0, color="k", ls="--", alpha=0.5)
# Add vertical lines at the max_frames times
for frame in max_frames_list:
    if frame < 95:  # only if frame is within your plotted range
        t = frame / 120
        plt.axvline(t, color="gray", ls=":", alpha=0.7)
plt.xlabel(r"Time Lag $\tau$ (s)")
plt.ylabel(r"$g_2(\tau) - 1$")
plt.title("Intensity Correlation Function (full range)")
plt.legend()
plt.tight_layout()
plt.show()
